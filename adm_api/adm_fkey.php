...
$query = mysql_query("select * from (select G2.usd_value,
...
from `adm_users` G1 LEFT JOIN `adm_user_data` G2 ON G1.usr_id = G2.usd_usr_id AND G2.usd_usf_id = 37 where G1.usr_id IN ( select T3.usr_id from `adm_users` T3 INNER JOIN `adm_members` T4 WHERE T3.usr_id = T4.mem_usr_id AND   T3.usr_valid = 1 AND   T4.mem_rol_id = 2 AND   T4.mem_end >= curdate() ) AND G2.usd_value IS NOT NULL) A where P IS NOT NULL;"); 
